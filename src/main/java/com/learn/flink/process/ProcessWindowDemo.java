...
public class ProcessWindowDemo {
    /**
     * 统计最近10秒钟内出现次数最多的两个水位并且每5秒钟更新一次
    ...
    public static class VcCount implements AggregateFunction<WaterSensor, Integer, Integer> {
    ...
    public static class TopN extends KeyedProcessFunction<Long, Tuple3<Integer, Integer, Long>, String> {
        ...
        MapState<Long, List<Tuple3<Integer, Integer, Long>>> mapState; //with State
        ...
        public void open(Configuration parameters) throws Exception {
            ...
            mapState = getRuntimeContext()
                    .getMapState(new MapStateDescriptor<>("mapState",
                                    Types.LONG,
                                    Types.LIST(
        ...
        public void processElement(Tuple3<Integer, Integer, Long> value, Context ctx, Collector<String> out) throws Exception {
            ...
            if (mapState.contains(windowEnd)) {
                // 1.1 包含vc，不是该vc的第一条，直接添加到List中
                List<Tuple3<Integer, Integer, Long>> dataList = mapState.get(windowEnd);
                dataList.add(value);
            } else {
                // 1.1 不包含vc，是该vc的第一条，需要初始化list
                List<Tuple3<Integer, Integer, Long>> dataList = new ArrayList<>();
                dataList.add(value);
                mapState.put(windowEnd, dataList);
            }
